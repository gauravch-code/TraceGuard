...
export async function GET() {
  ...
  return Response.json({ configured: Boolean(env.OPENAI_API_KEY) }, { headers: { "Cache-Control": "no-store" } });
}
...
export async function POST(request: Request) {
  ...
  const steps: SupportStep[] = [{ kind: "input", title: "Customer question received", detail: `Fictional order: ${input.daysSinceDelivery} days since delivery, $${input.amountUsd.toFixed(2)}.`, offsetMs: 0 }];
  try {
    ...
    return Response.json({ id, draft: result.draft, status: result.status, policy: result.policy }, { status: 201 });
  } catch (error) {
    ...
    return Response.json({ error: "The support agent could not finish this run. No refund action was taken." }, { status: 502 });
  }
}
